"""ARZ paper evaluation: HypNO-ARZ vs WENO5 vs Godunov, stratified by
(ic_type, num_segments) -- the ARZ analogue of scripts/paper_eval.py.

Mirrors the LWR paper-eval workflow so ARZ and LWR results are directly
comparable. Per (ic_type, num_segments) "cell" it computes per-channel MAE
(rho, w, v) vs the dataset's stored ground truth, classifies cells as ID/OOD
against the training config, and emits:

  * one per-cell qualitative figure per channel (rho, w): a u0/rho0 strip on
    top, a row of space-time heatmaps (GT | model | baselines), and a row of
    |solver - GT| heatmaps.
...
CSV and the per-channel appendix. Reuses the MAE/baseline/model-forward logic
from eval_vs_numerical_arz.
"""
...
def _locate_config(ckpt_path: Path, explicit: Path | None) -> Path | None:
    """Mirror load_hypno_arz_from_checkpoint's config auto-location."""
...
def _load_train_id_sets(
    ...
    if config_path is None or not Path(config_path).exists():
        print(f"[arz_paper_eval] no config for ID/OOD; all cells -> OOD")
        return set(), set()
    ...
    if not sec:
        print(
            f"[arz_paper_eval] config has no '{train_section}:' section; "
            f"all cells -> OOD"
        )
        return set(), set()
    ic_types = set(str(s) for s in sec.get("ic_types", []))
    segs = set(int(s) for s in sec.get("num_segments", []))
    print(f"[arz_paper_eval] train ic_types={sorted(ic_types)}  segs={sorted(segs)}")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="ARZ paper evaluation: HypNO-ARZ vs WENO5 vs Godunov, "
                    "stratified by (ic_type, num_segments)."
    )
    ap.add_argument("--ckpt", type=Path, required=True,
                    help="Checkpoint (legacy dict OR bare state_dict).")
    ...
    ap.add_argument("--out_dir", type=Path, default=None,
                    help="Output directory (default: <ckpt_dir>/arz_paper_eval).")
    args = ap.parse_args()
    ...
    print(f"[arz_paper_eval] pressure_form={get_pressure_form()}")
    ...
    print(f"[arz_paper_eval] loaded {args.ckpt}  tau_eval={tau}")
    ...
    out_dir = args.out_dir or (args.ckpt.parent / "arz_paper_eval")
    ...
    print(f"[arz_paper_eval] ic_types={ic_types}  num_segments={seg_values}")
    ...
    print(f"[arz_paper_eval] ID cells ({len(id_cells)}): {id_cells}")
    print(f"[arz_paper_eval] OOD cells ({len(ood_cells)}): {ood_cells}")
    ...
    print(f"\n[arz_paper_eval] all outputs in {out_dir}")
...
def _render_cell_figures(rep_by_cell, methods, x_np, t_np, figs_dir, plt):
    ...
    for (fam, seg), rep in rep_by_cell.items():
        ...
        for ch in panel_channels:
            ...
            fig.suptitle(
                f"arz_paper_eval sample {idx} | {format_ic_label(rho0, fam, seg)} "
                f"| channel={ch}\nMAE: {mae_str}",
                fontsize=11,
            )
        ...
        for m in methods:
            for ch in CHANNELS:
                ...
                wr.writerow(["ALL", "ALL", "ALL", m, ch,
                             f"{msd[0]:.6e}", f"{msd[1]:.6e}",
                             len(pooled_all[m][ch])])
    print(f"[arz_paper_eval] wrote {path}")
...
def _write_tex(out_dir, mae_by_cell, ic_types, seg_values, methods,
    ...
    ch = HEADLINE_CHANNEL
    lines: list[str] = []
    lines.append(f"% ARZ paper evaluation: MAE on {ch} (mean +/- std).")
    ...
    for fam in ic_types:
        ...
        lines.append(
            rf"\caption{{ARZ MAE on {ch}, mean $\pm$ std per cell. "
            rf"IC type: \texttt{{{fam}}}.}}"
        )
        lines.append(rf"\label{{tab:arz_paper_eval_{fam}}}")
    ...
    lines.append(r"\centering")
    lines.append(
        rf"\caption{{ARZ headline: MAE on {ch} (mean $\pm$ std). "
        r"ID = cells whose ic\_type and num\_segments both appear in training.}"
    )
    lines.append(r"\label{tab:arz_paper_eval_headline}")
    lines.append(r"\begin{tabular}{l" + "c" * len(methods) + "}")
    lines.append(r"\hline")
    ...
    lines.append(r"\centering")
    lines.append(
        rf"\caption{{ARZ appendix: per-num\_segments MAE on {ch}, pooled across "
        r"IC types. ID/OOD rows match Table~\ref{tab:arz_paper_eval_headline}.}"
    )
    lines.append(r"\label{tab:arz_paper_eval_appendix_seg}")
    lines.append(r"\begin{tabular}{l" + "c" * len(methods) + "}")
    lines.append(r"\hline")
    lines.append("num\\_segments & " + " & ".join(methods) + r" \\")
    lines.append(r"\hline")
    ...
    lines.append(r"\centering")
    lines.append(
        r"\caption{ARZ appendix: per-channel MAE (mean $\pm$ std), pooled by "
        r"ID/OOD and overall.}"
    )
    lines.append(r"\label{tab:arz_paper_eval_appendix_channels}")
...
def _write_txt(out_dir, mae_by_cell, ic_types, seg_values, methods,
    ...
    lines = ["arz_paper_eval -- MAE vs stored GT, mean +/- std (all channels)\n"]
    for fam in ic_types:
