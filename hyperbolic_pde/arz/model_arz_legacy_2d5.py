"""FROZEN legacy snapshot of model_arz.py at commit 4ec18fb ("pure pairwise").

DO NOT EDIT to match current code. This is the 2d+5 adj / 2d+3 nonadj edge
generation -- the architecture of the OLD checkpoints saved before commit
cde6c0a ("edge feature revision") trimmed adj to 2d+3. It exists ONLY to load
and inspect those pre-trim checkpoints (e.g. runs/hypno_arz_riemann.pt:
node_mlp=7, n_layers=8, d_hidden_nonadj=64, adj_msg=2d+5=197, nonadj=2d+3=195).

Class renamed HypNO_ARZ -> HypNO_ARZ_Legacy2d5, loader -> load_legacy2d5_from_
checkpoint, _cfg_to_kwargs -> _cfg_to_kwargs_legacy2d5, so it never collides with
the current model_arz.py on import. Body is otherwise verbatim from 4ec18fb.
...
This module also exposes `load_legacy2d5_from_checkpoint` -- a single helper
...
# Main model
# --------------------------------------------------------------------------- #
class HypNO_ARZ_Legacy2d5(nn.Module):
    ...
    def __init__(
        ...
        **_ignored,
        ...
        self.neighborhood_spacing = neighborhood_spacing
        if _ignored:
            print(f"[HypNO_ARZ_Legacy2d5] IGNORED kwargs = {sorted(_ignored.keys())}")
        print(
            f"[HypNO_ARZ_Legacy2d5] kx={stencil_k_x} kt={stencil_k_t} "
...
# --------------------------------------------------------------------------- #
def _cfg_to_kwargs_legacy2d5(model_cfg: dict) -> dict:
    """Translate a `hypno_arz` config block into HypNO_ARZ_Legacy2d5 constructor kwargs."""
...
def load_legacy2d5_from_checkpoint(
    ckpt_path,
    device: str = "cpu",
    config_path=None,
    model_section: Optional[str] = None,
):
    """Reconstruct a HypNO_ARZ_Legacy2d5 from a checkpoint file.
    ...
    Returns
    -------
    model : HypNO_ARZ_Legacy2d5 (on `device`, in eval mode)
    ...
    if is_legacy:
        ...
        model = HypNO_ARZ_Legacy2d5(**kwargs).to(device)
    ...
    kwargs = _cfg_to_kwargs_legacy2d5(model_cfg)
    model = HypNO_ARZ_Legacy2d5(**kwargs).to(device)
