"""FROZEN: the ORIGINAL (first) HypNO-ARZ -- the pre-pure-pairwise 2d+12 model.

Snapshot of model_arz.py at commit 11ae6bc (the LAST 2d+12 commit before the
pure-pairwise trims 4ec18fb/cde6c0a). This is the very first HypNO-ARZ edge
design (kitchen-sink adjacent edge: lam1_ij, lam2_ij, lam*_ij*r, drho, dv, dw,
theta, chi_1bad, chi_up1, chi_up2 -> 2d+12; nonadj 2d+4), but with the early
bug-fixes already in (upwind sign sigmoid(-...), CFL gate fix, p=rho switch,
use_relaxation_features toggle). Chosen over the raw first commit 42d5384 because
that one still had the upwind sign bug (fixed in db9c149).

Class renamed HypNO_ARZ -> HypNO_ARZ_Orig (and _ArzLifting/_ArzMPLayer ->
*Orig, loader/_cfg_to_kwargs -> *_orig) so it never collides with the current
model_arz.py on import. Body otherwise verbatim from 11ae6bc.

DO NOT EDIT to track current code -- this is a frozen baseline for training the
original architecture on the new stratified multi-discontinuity data.
...
This module also exposes `load_hypno_arz_orig_from_checkpoint` -- a single helper
...
# Lifting layer
# --------------------------------------------------------------------------- #
class _ArzLiftingOrig(nn.Module):
...
# --------------------------------------------------------------------------- #
class _ArzMPLayerOrig(nn.Module):
    ...
    def __init__(
        ...
        if shared_decoder is None:
            raise ValueError("_ArzMPLayerOrig requires a shared_decoder")
...
# Main model
# --------------------------------------------------------------------------- #
class HypNO_ARZ_Orig(nn.Module):
    ...
    def __init__(
        ...
        **_ignored,
        ...
        print(
            f"[HypNO_ARZ_Orig] kx={stencil_k_x} kt={stencil_k_t} "
...
# --------------------------------------------------------------------------- #
def _cfg_to_kwargs_orig(model_cfg: dict) -> dict:
    """Translate a `hypno_arz` config block into HypNO_ARZ_Orig constructor kwargs."""
...
def load_hypno_arz_orig_from_checkpoint(
    ckpt_path,
    device: str = "cpu",
    config_path=None,
    model_section: Optional[str] = None,
):
    """Reconstruct a HypNO_ARZ_Orig from a checkpoint file.
    ...
    Returns
    -------
    model : HypNO_ARZ_Orig (on `device`, in eval mode)
    ...
    if is_legacy:
        ...
        model = HypNO_ARZ_Orig(**kwargs).to(device)
    ...
    kwargs = _cfg_to_kwargs_orig(model_cfg)
    model = HypNO_ARZ_Orig(**kwargs).to(device)
