"""HypNO-ARZ Mark 2.1: two-family operator with ALL JUMPS IN BOTH BRANCHES.

This is mark2 with ONE change (see arz_mark2_1_all_jumps_2026-06-15 in the vault):
both family message MLPs now carry ALL THREE primitive jumps (drho, dv, dw) plus
BOTH interface speeds (s_1, s_2), instead of one-jump/one-speed per branch.

WHY: the w-invariant condition that keeps w two-valued is dw = dv + p'(rho)*drho.
mark2's split fed drho only to the GNL branch and dw only to the LD branch -- and
dv to NEITHER -- so no message MLP could compute that combination, and the
downstream sum/update sees only the collapsed (1-theta)*m_gnl + theta*m_ld. Each
branch having all three jumps lets it represent the invariant internally. Probe
for the spurious-3rd-w-band failure. The two-family split + soft theta-blend is
otherwise UNCHANGED (this is not a revert to the unified pre-pairwise MLP).
...
* Two characteristic wave families, separated AT THE MESSAGE:
    - 1-wave (GNL): forms shocks/rarefactions, preserves w. Carries the
      ...
      and an entropy gate (Oleinik/Lax) on its branch only.
    - 2-contact (LD): advects at v, preserves v. Carries s_2 = v_ij.
    - mark2.1: BOTH branches additionally carry (drho, dv, dw) + (s_1, s_2).
...
def _adj_interface_physics(rho_i, v_i, w_i, rho_j, v_j, w_j, rel_x):
    ...
    Returns dict with: s_1 (RH secant), s_2 (= v_ij), drho, dv, dw, theta,
    ...
    return {
        "s_1": s_1, "s_2": s_2, "drho": drho, "dv": dv, "dw": dw,
...
class _ArzLiftingM2(nn.Module):
    ...
    def __init__(
        ...
        # mark2.1 family edge MLPs: [h_i, h_j, s_1, s_2, drho, dv, dw, sign] -> 2d+6
        # (both branches get ALL jumps + both speeds, so each can represent the
        # w-invariant combination dw = dv + p'(rho)*drho).
        self.msg_gnl = _make_mlp(2 * d_latent + 6, d_hidden, d_latent, adj_depth, activation)
        self.msg_ld  = _make_mlp(2 * d_latent + 6, d_hidden, d_latent, adj_depth, activation)
    ...
    def forward(self, rho0, w0, x, t):
        ...
        def _adj_msg_gate(di, dm):
            ...
            # mark2.1: both branches carry all jumps (drho, dv, dw) + both
            # speeds (s_1, s_2). Same vector for GNL and LD; the split now
            # lives only in the SEPARATE WEIGHTS + the theta-blend, not in
            # which features each branch can see.
            edge_feats = torch.cat([
                ph["s_1"], ph["s_2"], ph["drho"], ph["dv"], ph["dw"],
                ph["sign_rel_x"],
            ], dim=-1)
            gnl_in = torch.cat([h_node, h_node, edge_feats], dim=-1)
            ld_in  = torch.cat([h_node, h_node, edge_feats], dim=-1)
...
class _ArzMPLayerM2(nn.Module):
    ...
    def __init__(
        ...
        # mark2.1: 2d+6 family MLPs (all jumps + both speeds in both branches).
        self.msg_gnl = _make_mlp(2 * d_latent + 6, d_hidden, d_latent, adj_depth, activation)
        self.msg_ld  = _make_mlp(2 * d_latent + 6, d_hidden, d_latent, adj_depth, activation)
        self.nonadj_msg = _make_mlp(2 * d_latent + 3, dh_na, d_latent, nonadj_depth, activation)
    ...
    def forward(self, h, x, t, rho0, w0):
        ...
        def _adj_msg_gate(di, dm):
            ...
            # mark2.1: both branches carry all jumps (drho, dv, dw) + both
            # speeds (s_1, s_2) -- same vector, separate weights + theta-blend.
            edge_feats = torch.cat([
                ph["s_1"], ph["s_2"], ph["drho"], ph["dv"], ph["dw"],
                ph["sign_rel_x"],
            ], dim=-1)
            gnl_in = torch.cat([h, h_j, edge_feats], dim=-1)
            ld_in  = torch.cat([h, h_j, edge_feats], dim=-1)
            m_gnl = self.msg_gnl(gnl_in)
            m_ld  = self.msg_ld(ld_in)
            theta = ph["theta"]
            g_ent = self._g_ent(ph["chi_1bad"])
...
# Main model
# --------------------------------------------------------------------------- #
class HypNO_ARZ_Mark2_1(nn.Module):
    """HypNO-ARZ Mark 2.1 operator (homogeneous ARZ, two-family, all-jumps).
    ...
    def __init__(
        ...
        **_ignored,
        ...
        if _ignored:
            print(f"[HypNO_ARZ_Mark2_1] IGNORED kwargs = {sorted(_ignored.keys())}")
        print(
            f"[HypNO_ARZ_Mark2_1] kx={stencil_k_x} kt={stencil_k_t} d_latent={d_latent} "
...
# --------------------------------------------------------------------------- #
def cfg_to_kwargs_m2_1(model_cfg: dict) -> dict:
...
def load_hypno_arz_mark2_1_from_checkpoint(
    ckpt_path, device: str = "cpu", config_path=None, model_section: str = "hypno_arz_mark2_1",
):
    """Reconstruct a HypNO_ARZ_Mark2_1 from a bare state_dict + run config.yaml."""
    import yaml
    ...
    model = HypNO_ARZ_Mark2_1(**cfg_to_kwargs_m2_1(cfg[model_section])).to(device)
