"""HypNO-ST v3 CharCone — ST3 with characteristic-aware non-adj edges and hyperbolic cone stencil.

Changes from hypno_st3.py
-------------------------
1. Non-adjacent / past message-passing edges are characteristic-aware.
   Two extra scalar features are appended to the non-adjacent edge input:
       v_edge  = rel_x / rel_t          (apparent edge propagation speed)
       char_res = v_edge - a_j          (residual vs characteristic speed at source)
   The non-adjacent MLP input dimension grows from 2d+11 to 2d+13.

2. The rectangular past candidate set is replaced by a hyperbolic cone.
   A new function `_enumerate_hyperbolic_cone_offsets` replaces
   `_enumerate_ball_offsets` everywhere in both the lifting layer and the
   MP layer.  The cone keeps only offsets satisfying
       |di| * dx  <=  c_max * |dm| * dt  +  buffer_cells * dx
   which is physically motivated by the maximum characteristic speed c_max=1
   for the LWR PDE.  Same-time adjacent edges (-1,0),(1,0) are always kept.
   Same-time non-adjacent spatial edges are excluded.

Everything else is identical to hypno_st3.py: adjacent edge logic, gate
formulas, decoder, skip connection, update net, lifting edge features,
checkpoint behaviour, forward signature, returned outputs, and public API.
"""
...
parser = argparse.ArgumentParser(description="Instantiate HypNO-ST v3 CharCone")
...
def _enumerate_hyperbolic_cone_offsets(
    k_x: int,
    k_t: int,
    dx: float,
    dt: float,
    ...
    """Hyperbolic-cone space-time stencil.

    Keeps same-time adjacent spatial edges (-1,0),(1,0) always, excludes
    same-time non-adjacent spatial edges, and for past edges (dm < 0) keeps
    only those satisfying:

        |di| * dx  <=  c_max * |dm| * dt  +  buffer_cells * dx

    When `causal=False`, falls back to `_enumerate_ball_offsets(k_x, k_t, causal)`.
    """
    if not causal:
        return _enumerate_ball_offsets(k_x, k_t, causal)

    out: list[tuple[int, int]] = []

    # Always include same-time adjacent spatial edges.
    out.append((-1, 0))
    out.append((1, 0))

    # Past edges filtered by cone condition.
    for dm in range(-1, -k_t - 1, -1):
        ell = abs(dm)
        max_di = int(c_max * ell * dt / dx + buffer_cells)
        max_di = min(k_x, max_di)
...
class _SpaceTimeLiftingLayer(nn.Module):
    """Space-time lifting over a hyperbolic-cone neighbourhood.

    A single unified edge MLP consumes edges `(di, dm)` inside the
    hyperbolic cone (same as ST3 except the rectangular stencil is replaced
    by the cone). Two gating modes are supported:
    ...
    * ``encoder_scaling = "physics"`` -- analytical physics gate.
    ...
    def forward(
        ...
        # --- Change 2: use hyperbolic cone offsets instead of ball ---
        offsets = _enumerate_hyperbolic_cone_offsets(
            k_x=k_x,
            k_t=k_t,
            dx=dx_grid,
            dt=dt_grid,
...
# --------------------------------------------------------------------------- #
# Physics-gated space-time MP layer (v3 CharCone)
# --------------------------------------------------------------------------- #
class _PhysicsSpaceTimeMPLayer(nn.Module):
    """Space-time MP over a hyperbolic-cone neighbourhood with analytical physics gate.
    ...
    * All other edges (non-adj spatial, temporal, diagonal): 2d+13 features,
      CFL gate only (plus optional char-cone).

    Change vs ST3: nonadj input adds v_edge and char_res (2 extra scalars).
    Change vs ST3: cone stencil replaces rectangular stencil.

    Adjacent edge features (2d + 10):
        h_i, h_j, u_i, u_j, f_i, f_j, a_i, a_j, a_ij, sign(a_ij), upwind, sign(rel_x)
    Non-adjacent edge features (2d + 13):
        h_i, h_j, u_i, u_j, f_i, f_j, a_i, a_j,
        rel_x, rel_t, v_edge, char_res, cfl, sign(rel_x), xi
    """
    ...
    def __init__(
        ...
        **_ignored,
        ...
        # nonadj: h_i, h_j, u_i, u_j, f_i, f_j, a_i, a_j,
        #         rel_x, rel_t, v_edge, char_res, cfl, sign(rel_x), xi = 2d+13
        # (Change 1: +2 vs ST3's 2d+11)
        self.nonadj_msg = _make_mlp(2 * d_latent + 13, dh_na, d_latent, 3, activation)
    ...
    def forward(
        ...
        def _build_edge(di: int, dm: int):
            """Return ``(msg_in, is_adj_sp, gate, rel_x, rel_t)`` for one cone edge."""
            ...
            if is_adj_sp:
                ...
                # --- Change 1: characteristic-aware non-adjacent edges ---
                a_ij = torch.zeros_like(rel_x)
                xi = x_i / t_i.clamp(min=1e-6)

                eps = 1e-6
                rel_t_safe = torch.where(
                    rel_t.abs() < eps,
                    torch.full_like(rel_t, -eps),
                    rel_t,
                )
                v_edge = rel_x / rel_t_safe
                char_res = v_edge - a_j
        ...
        if not nonadj_feats:
            raise RuntimeError(
                "No non-adjacent edges found in cone stencil. "
                "Increase k_t or buffer_cells."
            )
...
class HypNO_ST3(nn.Module):
    """HypNO-ST v3 CharCone — ST3 with characteristic-aware non-adj edges and cone stencil.

    Public API is identical to hypno_st3.HypNO_ST3; checkpoints are NOT
    compatible because nonadj_msg weights have a different input dimension.
    """
...
# Alias to allow importing under the new name too.
HypNO_ST3_CharCone = HypNO_ST3
