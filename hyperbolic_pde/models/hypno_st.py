...
class _SpaceTimeLiftingLayer(nn.Module):
    ...
    def __init__(self, d_latent: int, d_hidden: int, stencil_k: int, activation: str,
        ...
        self.edge_mlp  = _make_mlp(7, d_hidden, d_latent, 2, activation)   # [u0_i, u0_k, x_i, x_k, t_j, rel_x, |du0|]
        self.gate_net  = _make_mlp(7, d_hidden, 1,       2, activation)
    ...
    def forward(self, u0: torch.Tensor, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        # u0: [B, nx],  x: [B, nx],  t: [nt]
        B, nx = u0.shape
        nt = t.shape[0]
        ...
        # padded neighbours (spatial only — t enters via node/edge features)
        u_pad = F.pad(u0.unsqueeze(1), (k, k), mode="replicate").squeeze(1)  # [B, nx+2k]
        x_pad = F.pad(x.unsqueeze(1),  (k, k), mode="replicate").squeeze(1)  # [B, nx+2k]

        agg = torch.zeros_like(h_node)
        for j in range(-k, k + 1):
            u_k = u_pad[:, k + j : k + j + nx]   # [B, nx]
            x_k = x_pad[:, k + j : k + j + nx]   # [B, nx]

            u_k_bc = u_k.unsqueeze(1).unsqueeze(-1).expand(B, nt, nx, 1)
            x_k_bc = x_k.unsqueeze(1).unsqueeze(-1).expand(B, nt, nx, 1)
            rel_x  = x_k_bc - x_bc
            abs_du = (u0_bc - u_k_bc).abs()

            edge_in = torch.cat([u0_bc, u_k_bc, x_bc, x_k_bc, t_bc, rel_x, abs_du], dim=-1)  # [B,nt,nx,7]
...
class HypNO_ST(nn.Module):
    ...
    def forward(
        ...
        h = self.lifting(u0, x, t)                                          # [B, nt, nx, d]
