...
def create_run_dir(base: str = "hyperbolic_pde/runs/hypfluxno") -> Path:
...
# --------------------------------------------------------------------------- #
def hypfluxno_loss(
    pred: torch.Tensor,          # [B, nt, nx]
    target: torch.Tensor,        # [B, nt, nx]
    ...
    """Combined loss: L1 state + conservation + TV bound + entropy dissipation."""
    # L1 state loss (with optional shock weighting)
    if shock_weighted:
        du_dx = (target[:, :, 1:] - target[:, :, :-1]).abs()
    ...
    # mass conservation
    nx = pred.shape[2]
    loss_mass = ((pred.sum(dim=2) - target.sum(dim=2)).abs().mean()) / nx

    # TV bound: penalise predictions with higher TV than the target
    tv_pred = (pred[:, :, 1:] - pred[:, :, :-1]).abs().sum(dim=2)
    tv_target = (target[:, :, 1:] - target[:, :, :-1]).abs().sum(dim=2)
    loss_tv = torch.clamp(tv_pred - tv_target, min=0.0).mean()

    # Entropy dissipation loss
    # eta(u) = u^2 / 2 is the convex entropy; for physical solutions entropy
    # should not increase in time (Lax entropy condition).
    # Penalise positive increases in total entropy across time steps.
    if lambda_entropy > 0:
        eta = 0.5 * pred ** 2                             # [B, nt, nx]
        d_eta = eta[:, 1:, :] - eta[:, :-1, :]           # [B, nt-1, nx]
        entropy_increase = d_eta.sum(dim=-1)               # [B, nt-1] — sum over x
        loss_entropy = torch.clamp(entropy_increase, min=0.0).mean()
    else:
        loss_entropy = pred.new_tensor(0.0)
...
def setup_logging(run_dir: Path) -> logging.Logger:
    ...
    log = logging.getLogger("hypfluxno")
...
def main() -> None:
    ...
    lambda_entropy = float(model_cfg.get("lambda_entropy", 0.1))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for u0, u_full in loader:
            ...
            pred, beta_map = model(u0, x_grid, t_grid)
            loss, _ = hypfluxno_loss(
                pred, u_full,
                lambda_state, lambda_conservation, lambda_tv, lambda_entropy,
            ...
            if step % 50 == 0 or step == 1:
                ...
                msg = (
                    ...
                    f"tv={info['tv']:.3e}  entropy={info['entropy']:.3e} | lr={lr_now:.2e}"
                )
                if gpu:
                    msg += f" | {gpu}"
                log.info(msg)
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                for v_u0, v_u in val_loader:
                    v_u0 = v_u0.to(device)
                    v_u = v_u.to(device)
                    v_pred, _ = model(v_u0, x_grid, t_grid)
                    v_l, _ = hypfluxno_loss(
                        v_pred, v_u,
                        lambda_state, lambda_conservation, lambda_tv, lambda_entropy,
                        shock_weighted, shock_alpha,
                    )
            ...
            log.info(f"epoch {epoch}/{epochs} | val_loss={val_avg:.3e} | {gpu_status()}")
            for h in log.handlers:
                h.flush()
            model.train()
        else:
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("HypFluxNO training curves")
    ax.set_yscale("log")
    ax.legend()
