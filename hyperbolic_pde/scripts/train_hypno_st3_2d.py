"""Train HypNO-ST3-2D on the 2D LWR dataset.
...
The conservation term is the integral form of the conservation law -- it
sums u (no derivatives), so it stays valid across shocks. The target is the
IC's exact total mass M_ic, not zero.

Usage from repo root:
    python hyperbolic_pde/scripts/train_hypno_st3_2d.py
    python hyperbolic_pde/scripts/train_hypno_st3_2d.py --config <yaml>
    python hyperbolic_pde/scripts/train_hypno_st3_2d.py --resume_run <run_dir>
"""
...
# dataset
# --------------------------------------------------------------------------- #
class LWR2DDataset(Dataset):
    """Each __getitem__ returns (u0, u_full): u0 [ny,nx], u_full [nt,ny,nx]."""
...
def setup_logging(run_dir: Path) -> logging.Logger:
    log = logging.getLogger("hypno_st3_2d")
...
def _find_latest_checkpoint(run_dir: Path) -> tuple[Path | None, int]:
    ...
    # as `checkpoint_epoch{epoch}.pt` without zero padding — lex would pick
...
# --------------------------------------------------------------------------- #
def lwr_2d_loss(
    pred: torch.Tensor,            # [B, nt, ny, nx]
    target: torch.Tensor,         # [B, nt, ny, nx]
    u0: torch.Tensor,             # [B, ny, nx]
    ...
    # Option A: temporal mass conservation. M_pred(t) should equal M_ic.
    # Sum over the two spatial axes (per-sample, per-timestep totals).
    m_pred = pred.sum(dim=(2, 3))                       # [B, nt]
    m_ic = u0.sum(dim=(1, 2)).unsqueeze(1)              # [B, 1]
...
# --------------------------------------------------------------------------- #
def main() -> None:
    parser = argparse.ArgumentParser(description="Train HypNO-ST3-2D on the 2D LWR dataset.")
    ...
    print("--- STARTING HYPNO_ST3_2D TRAINING ---")
    ...
    if resume_run_dir:
        ...
        run_dir = create_run_dir(model_cfg.get("run_base", "hyperbolic_pde/runs/hypno_st3_2d"))
    ...
    train_data = LWR2DDataset(bundle.u0[train_idx], bundle.u[train_idx])
    loader = DataLoader(train_data, batch_size=int(model_cfg["batch_size"]), shuffle=True)
    val_loader = None
    if val_idx.size > 0:
        val_data = LWR2DDataset(bundle.u0[val_idx], bundle.u[val_idx])
    ...
    lambda_conservation = float(model_cfg.get("lambda_conservation", 0.1))
    ...
    fig.suptitle("HypNO-ST3-2D training curves")
