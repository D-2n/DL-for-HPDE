...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train FNO experiment on hyperbolic PDE dataset.")
    parser.add_argument(
        "--config",
        type=str,
    ...
    if epochs is None:
        if "steps" in fno_cfg:
            ...
            print(
                f"[FNO-EXP] config uses steps={steps}; converting to epochs={epochs} "
                f"(steps/epoch={steps_per_epoch})."
            )
        else:
            raise KeyError("fno_experiment config must define 'epochs' (or legacy 'steps')")
    epochs = int(epochs)
    ...
    for epoch in range(1, epochs + 1):
        for inp, out in loader:
            ...
            if step % 100 == 0 or step == 1:
                ...
                print(
                    f"[FNO-EXP] epoch {epoch:3d}/{epochs} | step {step:5d}/{total_steps} | "
        ...
        if val_loader is not None:
            ...
            print(f"[FNO-EXP] epoch {epoch:3d}/{epochs} | val_mse={val_mse:.3e}")
            model.train()
        ...
        if checkpoint_every > 0 and epoch % checkpoint_every == 0:
            ...
            print(f"[FNO-EXP] Saved checkpoint to {ckpt_path}")

    save_path = Path(fno_cfg["save_path"])
    save_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), save_path)
    elapsed = time.perf_counter() - start_time
    print(f"Saved FNO-EXP checkpoint to {save_path}")
    print(f"[FNO-EXP] Training time: {elapsed:.2f}s")
