"""256x256 OOD comparison: HypNO-ST3 vs WENO5 vs Godunov, zero-shot resolution.

Identical to final_comparison.py but evaluates a trained (128x128) model on a
256x256 OOD set produced by generate_ood_data_256.py. The grid (x range, nt,
t_max) is read directly from the dataset's own x/t arrays, so this script needs
no resolution flags — it adapts to whatever 256 set is passed.

Loads a trained model from --run-dir, runs all three solvers on every sample of
the grouped 256 OOD dataset, and reports:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to the 256 OOD .npz. Overrides --block.",
    )
    parser.add_argument(
        "--block", type=str, default="ood_data_256_wide",
        help="Config block whose 'path' gives the 256 OOD .npz when --data_path "
        "is not set (e.g. ood_data_256_wide, ood_data_256_train).",
    )
    parser.add_argument(
    ...
    # ---- dataset ----
    # The run config.yaml is a training-time snapshot and may not carry the
    # ood_data_256_* blocks. Fall back to the repo base config for --block.
    if args.data_path:
        data_path = Path(args.data_path)
        block_tag = Path(args.data_path).stem
    else:
        block_cfg = cfg.get(args.block)
        if block_cfg is None:
            base_path = ROOT / "configs" / "hyperbolic_pde.yaml"
            with base_path.open("r", encoding="utf-8") as f:
                block_cfg = yaml.safe_load(f).get(args.block)
        if not block_cfg or "path" not in block_cfg:
            raise KeyError(
                f"No --data_path given and config block '{args.block}' has no "
                f"'path'. Pass --data_path explicitly."
            )
        data_path = Path(block_cfg["path"])
        block_tag = args.block
    print(f"Loading 256 OOD dataset from {data_path}")
    ...
    plot_dir = run_dir / f"plots_final_comparison_256_{block_tag}"
