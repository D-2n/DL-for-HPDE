...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate FNO experiment on test set.")
    parser.add_argument(
        "--config",
        type=str,
    ...
    plot_dir = Path(fno_cfg.get("plot_dir", "hyperbolic_pde/runs/plots/fno_experiment"))
        ...
        for inp, out in loader:
            ...
            if plots_made < int(fno_cfg.get("eval_plots", 3)):
                for b in range(pred.size(0)):
                    ...
                    axes[0].set_title("FNO-EXP prediction")
                    axes[0].set_xlabel("x")
                    axes[0].set_ylabel("t")
                    ...
                    out_path = plot_dir / f"fno_experiment_sample_{plots_made}.png"
                    fig.savefig(out_path, dpi=150)
                    plt.close(fig)
                    plots_made += 1

    mse = total_loss / max(1, total_count)
    print(f"[FNO-EXP] Test MSE: {mse:.6e}")
