...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate FNO experiment on 1x and 2x test data.")
    parser.add_argument(
        "--config",
        type=str,
    ...
    plot_dir = Path("hyperbolic_pde/runs/plots/fno_experiment_superres")
    ...
    with torch.no_grad():
        for idx in range(min(max_plots, u1.shape[0], u2.shape[0])):
            ...
            axes[0, 0].set_title("FNO-EXP prediction (1x)")
            ...
            axes[1, 0].set_title("FNO-EXP prediction (2x)")
            ...
            out_path = plot_dir / f"fno_experiment_superres_{plots_made}.png"
            fig.savefig(out_path, dpi=150)
            plt.close(fig)
            plots_made += 1

    print(f"[FNO-EXP] Test MSE 1x: {mse_1x:.6e}")
    print(f"[FNO-EXP] Test MSE 2x: {mse_2x:.6e}")
    print(f"[FNO-EXP] Saved {plots_made} plots to {plot_dir}")
