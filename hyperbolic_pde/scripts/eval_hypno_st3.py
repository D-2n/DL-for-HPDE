"""Evaluate HypNO-ST v3 on test set."""
...
from hyperbolic_pde.models.hypno_st3 import HypNO_ST3
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate HypNO-ST3 on test set.")
    parser.add_argument(
        "--config", type=str,
    ...
    if run_dir and (run_dir / "config.yaml").exists():
        with (run_dir / "config.yaml").open("r", encoding="utf-8") as f:
            cfg = yaml.safe_load(f)
        print(f"[HypNO-ST3] Using config from {run_dir / 'config.yaml'}")
    else:
    ...
    print(f"[HypNO-ST3] {n_params:,} trainable parameters")

    if run_dir:
        plot_dir = run_dir / "plots"
    else:
        plot_dir = Path(model_cfg.get("plot_dir", "hyperbolic_pde/runs/plots/hypno_st3"))
    plot_dir.mkdir(parents=True, exist_ok=True)
    print(f"[HypNO-ST3] Saving plots to {plot_dir}")
    ...
    print(f"[HypNO-ST3] Test MSE:  {mse:.6e}")
    print(f"[HypNO-ST3] Test MAE:  {mae:.6e}")
    print(f"[HypNO-ST3] Test rL2:  {rel_l2:.6e}")
    ...
    if mask_pos.any() and mask_neg.any():
        ...
        print(f"[HypNO-ST3] MAE (correction >= 0): {mae_pos:.6e}")
        print(f"[HypNO-ST3] MAE (correction <  0): {mae_neg:.6e}")
        print(f"[HypNO-ST3] Ratio neg/pos:         {mae_neg / mae_pos:.3f}")
    ...
    fig_err.savefig(plot_dir / "hypno_st3_error_vs_time.png", dpi=150)
    plt.close(fig_err)
    ...
    for b in range(n_test):
        ...
        axes[0].set_title("HypNO-ST3 prediction")
        axes[0].set_xlabel("x")
        axes[0].set_ylabel("t")
    ...
    print(f"[HypNO-ST3] Saved {plots_made} plots + error curve to {plot_dir}")
    print(f"[HypNO-ST3] Metrics saved to {metrics_path}")
