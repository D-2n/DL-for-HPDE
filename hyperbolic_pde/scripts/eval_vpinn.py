...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate VPINN on test set.")
    parser.add_argument(
        "--config",
        type=str,
    ...
    plot_dir = Path(vpinn_cfg.get("plot_dir", "hyperbolic_pde/runs/plots/vpinn"))
    plot_dir.mkdir(parents=True, exist_ok=True)

    eval_samples = int(vpinn_cfg.get("eval_samples", 5))
    ...
    with torch.no_grad():
        for idx in eval_idx:
            ...
            mse_list.append(mse)
            print(f"[VPINN] sample {int(idx)} MSE: {mse:.6e}")

            if plots_made < int(vpinn_cfg.get("eval_plots", 3)):
                ...
                axes[0].set_title("VPINN prediction")
                axes[0].set_xlabel("x")
                axes[0].set_ylabel("t")
                ...
                out_path = plot_dir / f"vpinn_sample_{plots_made}.png"
                fig.savefig(out_path, dpi=150)
                plt.close(fig)
                plots_made += 1

    if mse_list:
        print(f"[VPINN] mean MSE: {float(np.mean(mse_list)):.6e}")
    print(f"[VPINN] Saved {plots_made} plots to {plot_dir}")
